...
export default function LoginPage() {
  ...
  const { login, signInWithGoogle, resendConfirmation } = useAuth()
  ...
  const handleResend = async () => {
    const email = notConfirmedEmail || emailValue
  ...
  return (
    ...
    >
      <div className="rounded-2xl border border-border/60 bg-card/80 px-8 py-8 shadow-2xl backdrop-blur-xl">
        <div className="mb-6">
          <h1 className="text-xl font-semibold tracking-tight text-foreground">Seja bem-vindo</h1>
          <p className="mt-1 text-sm text-muted-foreground">Entrar com suas credências</p>
        </div>

        {/* Google */}
        <button
          type="button" onClick={handleGoogle} disabled={busy}
          className="mb-4 flex w-full items-center justify-center gap-2.5 rounded-lg border border-border bg-secondary/50 px-4 py-2.5 text-sm font-medium text-foreground transition-all hover:bg-secondary active:scale-[0.99] disabled:opacity-60 disabled:cursor-not-allowed"
        >
        ...
        <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-4">
          <div>
            ...
            <input
              {...register('email')}
              type="email" placeholder="exemplo@email.com" autoComplete="email" disabled={busy}
              className={cn(
          ...
          {/* Email not confirmed — inline alert */}
          {notConfirmedEmail && (
            <motion.div
            ...
            >
              <p className="text-xs font-medium text-amber-400">Email não confirmado</p>
              <p className="text-[11px] text-amber-400/80 leading-relaxed">
                Verifique seu email <strong>{notConfirmedEmail}</strong> e clique no link de confirmação.
              </p>
              <button
                type="button" onClick={handleResend} disabled={resending}
                className="flex items-center gap-1.5 text-[11px] font-medium text-amber-400 hover:text-amber-300 transition-colors disabled:opacity-60"
              >
                {resending
                  ? <><Loader2 size={11} className="animate-spin" />Enviando…</>
                  : <><RefreshCw size={11} />Reenviar a confirmação de Email</>}
              </button>
            </motion.div>
          )}
      ...
      <p className="mt-4 text-center text-sm text-muted-foreground">
        Não tem uma conta?{' '}
        <Link href={ROUTES.register} className="font-medium text-foreground hover:text-primary transition-colors">
          Criar conta
        </Link>
      </p>
    </motion.div>
